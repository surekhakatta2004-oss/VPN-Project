...
from src.config import VPN_CLASSIFIER_DIR

class VPNClassifier:
    """
    VPN Classifier wrapper. Automatically loads the application classification model,
    scaler, label encoder, and feature columns to classify VPN traffic into specific applications.
    """
    def __init__(self, model_dir: Path = None):
        if model_dir is None:
            model_dir = VPN_CLASSIFIER_DIR
        
        self.model_dir = Path(model_dir)
        self.feature_columns = load_json(self.model_dir / "feature_columns.json")
        self.scaler = load_pickle(self.model_dir / "vpn_application_scaler.pkl")
        self.label_encoder = load_pickle(self.model_dir / "vpn_application_label_encoder.pkl")
        self.model = load_pickle(self.model_dir / "best_vpn_application_model.pkl")

    def predict(
        self, data: Union[pd.DataFrame, Dict[str, Any], List[Dict[str, Any]]]
    ) -> Tuple[np.ndarray, np.ndarray]:
        """
        Classifies VPN traffic flow into specific applications.
